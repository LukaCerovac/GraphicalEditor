...
class Commands
  ...
  def color_region(x, y, color, image)
    ...
    raise_error(ERROR_MSG[:invalid_x_input]) if x_input.any? {|x| x<1 || x>m}
    raise_error(ERROR_MSG[:invalid_y_input]) if y_input.any? {|y| y<1 || y>n}
